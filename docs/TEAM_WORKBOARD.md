...
| P-04 | Add evaluator runner and failure-path tests | Palak | IN_PROGRESS | `codex/palak-evaluator-integration-tests` | `evaluation/deepfake/tests/test_evaluator_integration.py`; cover `evaluate_manifest`, output files, no-face/read errors and invalid thresholds | 2026-09-11 | Claim is present on `origin/main`; isolated evaluator tests are being implemented |
